...
- `src/ovp_pipeline/autopilot/queue.py`
...
- local clipping note: [Creating a Second Brain with Claude Code.md](/Users/chris/Documents/ovp-vault/Clippings/Creating%20a%20Second%20Brain%20with%20Claude%20Code.md)
...
- [README.md](/Users/chris/Documents/obsidian-vault-pipeline/README.md)
- [docs/pack-api/README.md](/Users/chris/Documents/obsidian-vault-pipeline/docs/pack-api/README.md)
- [src/ovp_pipeline/truth_store.py](/Users/chris/Documents/obsidian-vault-pipeline/src/ovp_pipeline/truth_store.py)
- [src/ovp_pipeline/autopilot/queue.py](/Users/chris/Documents/obsidian-vault-pipeline/src/ovp_pipeline/autopilot/queue.py)
- [src/ovp_pipeline/commands/ui_server.py](/Users/chris/Documents/obsidian-vault-pipeline/src/ovp_pipeline/commands/ui_server.py)
...
- [Creating a Second Brain with Claude Code.md](/Users/chris/Documents/ovp-vault/Clippings/Creating%20a%20Second%20Brain%20with%20Claude%20Code.md)
...
- plugin hooks 在 Claude Code / OVP / OpenCode / Codex CLI 上自动捕获与召回
...
- `/tmp/zilliztech-memsearch/docs/platforms/ovp/how-it-works.md`
...
- `/tmp/zilliztech-memsearch/plugins/ovp/README.md`
- `/tmp/zilliztech-memsearch/plugins/ovp/index.ts`
- `/tmp/zilliztech-memsearch/plugins/ovp/scripts/derive-collection.sh`
...
README、OVP integration 文档和源码都指向同一个中心：
...
`hooks/session-start.ts`、README 的 hook 表和 OVP integration 说明：
...
- `/tmp/agentmemory/integrations/ovp/README.md`
...
- use case：Claude Code plugin、OVP plugin、demo apps
...
`examples/ovp-plugin/README.md` 很关键：
...
- `/tmp/EverOS/methods/evermemos/examples/ovp-plugin/README.md`

## Round 20: `HKUDS/DeepTutor`

### What this project actually is

`DeepTutor` 值得看，但要把对象看准。

它不是一个“知识抽取系统”，也不是一个 OVP 意义上的 knowledge compiler。

它更准确的定位是：

**一个把 KB-RAG、轻量 memory、notebook grounding、以及 learning/research products 拼成 tutoring workflow 的 agent-native learning system。**

换句话说，它的主问题是：

- 怎么把用户资料变成可检索的知识库
- 怎么在一次 tutoring / research turn 里把 KB、history、memory、notebook context 拼起来
- 怎么把检索到的材料编译成 report、guided learning page、quiz、solve result

而不是：

- 怎么把 source absorb 成 canonical objects / claims / evidence
- 怎么维护长期 truth evolution
- 怎么做 reviewable knowledge compilation

### The implemented knowledge path

先看最关键的 KB 入口。

`deeptutor/knowledge/initializer.py` 和 `deeptutor/knowledge/add_documents.py` 的实现非常直接：

- 建 `raw/` + `llamaindex_storage/`
- 复制原文件到 `raw/`
- 走 `RAGService.initialize(...)`
- 由 `LlamaIndexPipeline` 做解析、chunking、embedding、持久化索引

而 `deeptutor/services/rag/pipelines/llamaindex.py` 也说明了它到底做了什么：

- PDF -> `PyMuPDF` 抽纯文本
- text/markdown/code/config -> 直接读文本
...
**raw docs -> text extraction -> fixed chunking -> embedding -> vector index**

这条链没有更深的知识抽取层。

### The strongest negative signal: structured extraction was explicitly removed

这个仓库里其实保留了一个很关键的历史痕迹：它曾经想做更细粒度结构化抽取，但现在主动退掉了。

证据很明确：

- `KnowledgeBaseInitializer.extract_numbered_items()` 是 no-op
- `DocumentAdder.extract_numbered_items_for_new_docs()` 也是 no-op
- `services/rag/components/chunkers/numbered_item.py` 里虽然还保留了 “definitions / theorems / equations” 抽取器接口，但运行时直接返回空，并明确记录：
  - `deprecated and disabled in llamaindex-only mode`
- `services/rag/factory.py` 也说明现在 provider 已经被收缩到单一 `llamaindex`

这很重要，因为它告诉我们：

**DeepTutor 当前的核心路线不是“从文档里抽结构化知识”，而是“保持索引层简单，把智能放在检索后面的 tutoring / research orchestration”。**

### Where the real value is: context assembly

`deeptutor/services/session/turn_runtime.py` 是这轮最值得看的文件。
...
然后统一塞进 `UnifiedContext`。

这里有两个比 KB 本身更有意思的点。

第一，**notebook grounding 不是简单拼文本，而是先过一个 NotebookAnalysisAgent**：

- thinking
- acting（选最相关 records）
- observing（产出紧凑观察总结）

也就是说，它不是直接把所有 notebook record 灌进 prompt，而是先做一次针对当前问题的 notebook-side context compression。

第二，**memory 也是单独维护的轻量 context layer**：

- `SUMMARY.md`
- `PROFILE.md`

`services/memory/service.py` 里会基于最近 turn 重写这两个文件，再把它们通过 `build_memory_context()` 注入。

所以 `DeepTutor` 的真实强项，不是 absorb pipeline，而是：

**一次 agent turn 之前，如何把多个 context surfaces 压成一个可用上下文。**

### Notebooks and compiled learning products

`services/notebook/service.py` 的 notebook 模型也说明了系统的产品中心。

notebook record 里存的是：
...
它不是 canonical knowledge object，而是：

**针对未来复用的 saved work artifact。**

`NotebookSummarizeAgent` 会把一条产物压成 80-180 词摘要，强调：
...
这又进一步说明它在做的是“结果复用”和“学习工作台”，不是 truth modeling。

`deep_research` / `ResearchPipeline` 也一样：

- 先 rephrase / decompose
- 再 research
- 再 note summarize
- 最后 reporting

而且 `request_config.py` 里把 mode 明确成：
...
**从 evidence 到不同学习产物形状的编译。**

### Is there any real extraction logic at all?

有，但不在通用 KB 主链路里。

比如 `tools/question/question_extractor.py` 确实是个真实的 LLM-based structured extraction pipeline：

- 读 MinerU 解析后的 markdown
- 识别题目
- 输出结构化 JSON

但这更像一个 domain-specific tool，不是整个 DeepTutor 的核心知识架构。

所以更准确的判断是：

- 通用知识库主链路：**没有强知识抽取，只有文本索引**
- 某些专业功能：**有单点结构化抽取工具**

### What is useful for OVP

对 OVP 来说，它有价值的不是 KB 层，而是这几件事：

#### 1. Context assembly before the main capability

`turn_runtime.py` 这种统一拼装：
...
**真正有价值的“处理”不一定发生在 ingest 时，也可以发生在 turn-time assembly 时。**

OVP 以后做 orientation brief / object brief / operator briefing 时，这个思路很有用。

#### 2. Notebook-side compression before injection

`NotebookAnalysisAgent` 这条链值得注意：

- 不是把选中 records 直接塞给主 agent
- 而是先针对当前问题做一次 notebook reasoning + selection + observation

这和我们前面说的 `orientation artifact` 很接近。

OVP 可以借这个思想做：

- object-page grounding
- topic-overview grounding
- selected-artifact briefing

#### 3. Treat outputs as reusable work artifacts

DeepTutor 的 notebook 不是 truth store，而是 reusable work artifacts store。

这对 OVP 也有启发：

**compiled artifact 和 canonical artifact 应该明确区分。**

OVP 的 `orientation brief / overview / dossier` 更接近这里的 notebook records，而不是 object/claim/evidence。

### What OVP should not borrow

#### 1. 不要把 KB 索引当成知识处理

DeepTutor 的 KB pipeline 做得干净，但本质还是：
...
这对 tutoring 足够，但对 OVP 的知识编译目标不够。

#### 2. 不要把 notebook record 当 canonical truth
...
但这些是 work artifacts，不是 canonical truth graph。

#### 3. 不要把 lightweight memory 误当长期知识层

`SUMMARY.md + PROFILE.md` 很适合作为 tutoring personalization memory，
不适合作为 OVP 的长期知识底座。

### Working conclusion from Round 20

`DeepTutor` 对 OVP 有帮助，但帮助点很收敛：

**它对“知识抽取”本身借鉴不大；对“检索后的上下文装配”和“把材料编译成学习/工作产物”更有启发。**

一句话压缩：

**`DeepTutor` 更像一个 tutoring-oriented context assembly and product generation system，不是一个强知识抽取系统。**
...
- `/tmp/DeepTutor/deeptutor/knowledge/initializer.py`
- `/tmp/DeepTutor/deeptutor/knowledge/add_documents.py`
- `/tmp/DeepTutor/deeptutor/knowledge/manager.py`
- `/tmp/DeepTutor/deeptutor/services/rag/service.py`
- `/tmp/DeepTutor/deeptutor/services/rag/pipelines/llamaindex.py`
- `/tmp/DeepTutor/deeptutor/services/rag/components/chunkers/numbered_item.py`
- `/tmp/DeepTutor/deeptutor/services/session/turn_runtime.py`
- `/tmp/DeepTutor/deeptutor/services/memory/service.py`
- `/tmp/DeepTutor/deeptutor/services/notebook/service.py`
- `/tmp/DeepTutor/deeptutor/agents/notebook/analysis_agent.py`
- `/tmp/DeepTutor/deeptutor/agents/notebook/summarize_agent.py`
- `/tmp/DeepTutor/deeptutor/capabilities/deep_research.py`
- `/tmp/DeepTutor/deeptutor/agents/research/research_pipeline.py`
- `/tmp/DeepTutor/deeptutor/agents/research/agents/note_agent.py`
- `/tmp/DeepTutor/deeptutor/agents/research/request_config.py`
- `/tmp/DeepTutor/deeptutor/tools/question/question_extractor.py`
