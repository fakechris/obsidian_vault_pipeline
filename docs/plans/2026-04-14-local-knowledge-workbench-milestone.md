...
- `Phase 25` is now the next planned narrowing slice:
  - canonical run ledger as the single source of truth for active workflow state
  - observable-first runtime semantics with heartbeat, work-unit progress, and current-item visibility
  - one runtime interpretation shared by watcher, API, and UI
- `Phase 24` remains planned immediately after `Phase 25`:
  - brain-first lookup before downstream object/link creation
  - backlink legibility after downstream creation/linking
...
- `Milestone 7: Active Signal Loop`
  - next execution slice should focus on runtime observability and a canonical run ledger
  - `Phase 24` should wait until the pipeline is no longer a black box at operator time
...
- the next phase should make workflow state itself trustworthy:
  - one canonical run ledger
  - explicit active vs stale vs blocked semantics
  - counted progress where a real denominator exists
  - current-item visibility for long-running stages
- once that lands, the following phase should tighten brain-first lookup before object/link creation
- newly created downstream objects should then make backlink expectations more explicit
...
- use the next Milestone 7 slice for runtime observability + canonical run truth
- use the following Milestone 7 slice for brain-first lookup + backlink legibility instead of reopening shell UX or widening immediately into background intelligence
