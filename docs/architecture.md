...
The write/apply/derived cycle (raw → note → evergreen **stub** → canonical → MOC + knowledge index) is closed, assembled declaratively, and runs end-to-end from one `run-cycle` command (idempotent on re-run). The legacy **absorb/crystal loop** is **not yet implemented** — minting writes only a provenance-free stub, with no note enrichment, policy lanes, cross-document dedup, or crystal materialization (see `docs/processing-pipeline-audit.md` and the M12 recommendation below).
