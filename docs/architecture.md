...
| **L5 Read / health** *(in progress)* | `ovp-query` ✅, `ovp-lint` *(next)* | read canonical / vault / knowledge index. | own mutation or pipeline execution |
...
- **`ovp-query`** — the read layer (L5). A read-only `KnowledgeView` loads the canonical store (authority, strict parse) + the derived knowledge index (backlinks, if present) and answers `concepts` / `get` / `search` / `backlinks` / `stats`. Fail-loud on a corrupt store; never mutates, assembles, or runs. `ovp-lint` (next) builds health checks on the same `KnowledgeView`. Depends on `ovp-domain`, `ovp-stores`, `ovp-core` — not `ovp-app`/`ovp-run`. See `docs/stage-read-health.md`.

- **`ovp-cli`** — thin app layer: parses args, builds the `ModelClient` + `ConceptRegistry` + `AppWiring` + paths, and delegates assembly to `ovp-app::GraphAssembler` and the full cycle to `ovp-run::RunCycle` (no hand-wired `register_*` on the main path — CI-gated). No business logic. Subcommands: `run-cycle` (the L4 operational command), `query` (the L5 read command), `interpret-article` (the v1 pipeline, assembled), `apply-plan` (`WritePlan` → vault), `graph` (manifest inspection). `run --fake` remains from v0.1 for fake-source smoke tests.
...
**Next:** finish the **L5 read/health** layer. `ovp-query` (a read-only `KnowledgeView` + `query` CLI) is landed; next is `ovp-lint` — WIGS-style checks over canonical + vault (missing evergreen notes, stale index/MOC, broken `[[wikilinks]]`, orphan canonical records), reporting findings without ever fixing (a fix is a write; writes go through L3/L4). Then **L6** — a real RAG retriever + ranking/context builder + eval, and an autopilot watcher that *calls* the L4 `run-cycle` rather than duplicating workflow logic. Re-triaged from `docs/legacy-alignment.md` against observed pain.
