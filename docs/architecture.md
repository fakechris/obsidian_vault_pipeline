...
- **`ovp-run`** — the operational workflow layer (L4). One public concept, `RunCycle`, drives a full cycle: assemble (L2) → run (L0) → apply the plan via `CompositePlanApplier` (L3) → read the canonical store strictly → rebuild the MOC and knowledge index → one `RunCycleReport`. Idempotent on re-run; fail-closed (assembly/run failure → no derived rebuild; corrupt canonical → MOC/index left untouched). Owns no domain logic — it wires L1–L3 together. Depends on `ovp-app`, `ovp-stores`, `ovp-domain`, `ovp-core` (not `ovp-llm` — the client is injected via `AppWiring`). See `docs/stage-operational-workflow.md`.

- **`ovp-cli`** — thin app layer: parses args, builds the `ModelClient` + `ConceptRegistry` + `AppWiring` + paths, and delegates assembly to `ovp-app::GraphAssembler` and the full cycle to `ovp-run::RunCycle` (no hand-wired `register_*` on the main path — CI-gated). No business logic. Subcommands: `run-cycle` (the L4 operational command), `interpret-article` (the v1 pipeline, assembled), `apply-plan` (`WritePlan` → vault), `graph` (manifest inspection). `run --fake` remains from v0.1 for fake-source smoke tests.
...
**Next:** the **L5 read/health** surfaces — `ovp-query` (a read surface over the knowledge index) and `ovp-lint` (WIGS-style checks over canonical + vault); they read, they never mutate. Then **L6** — a real RAG retriever + ranking/context builder + eval, and an autopilot watcher that *calls* the L4 `run-cycle` rather than duplicating workflow logic. Re-triaged from `docs/legacy-alignment.md` against observed pain.
