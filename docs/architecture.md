...
3. **L4/L5 MOC + knowledge index** ✅ *(done)*. `MocBuilder` (Atlas MOC index, derived from the canonical store) and `KnowledgeIndexBuilder` (concepts + vault backlinks, derived from canonical + a `[[wikilink]]` scan via `ovp-stores::walk_markdown`). Both are pure builders that emit `VaultCreate`/`VaultUpdate` rebuild plans (Create when absent, Update-with-`before_hash` when changed, no-op when unchanged), applied through `VaultFsPlanApplier`. Rebuildable from canonical + vault alone (invariant #11), tested end-to-end and idempotent. **`TxnFsApplier` assessed and not built:** multi-file atomicity is not actually required — every op is idempotent, so a partial apply is recovered by re-running (no rollback needed). It is added only if a future operation genuinely needs all-or-nothing multi-file semantics.

The full legacy cycle (raw → note → evergreen → canonical → MOC + knowledge index) is now closed. Next work is re-triaged from `docs/legacy-alignment.md` P1 (e.g. `ovp-query` over the knowledge index, `ovp-lint`, autopilot) against observed pain.
