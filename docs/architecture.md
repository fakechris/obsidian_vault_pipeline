...
**M12a — rich evergreen minting: landed.** Closing the biggest gap the M11 necessity audit (`docs/processing-pipeline-audit.md`) found, `EvergreenConceptWriter` + `EvergreenSink` now mint a **grounded** note (one-line definition + 2-5 source-backed claims + a source link + related wikilinks, selected deterministically from the interpreted article) instead of a provenance-free stub, so RAG retrieves over real content rather than placeholders. The grounding lives in the vault note body only; the canonical store / MOC / knowledge index are unchanged. See `docs/stage-m12a-rich-evergreen-minting.md`.

**Recommended next stage: M12b — Absorb Boundary v2, before RAG v1.1 (the semantic ranker).** M12a is mint-*new*-only with an AUTO-all policy. Still future, in order: enrichment of *existing* evergreen notes; mint/enrich/escalate/reject policy lanes; and cross-document merge/dedup of a shared slug — until M12b, a second distinct article surfacing the same new slug fails its `VaultCreate` loud (no overwrite) rather than merging, a known limitation pinned by an ovp-stores e2e test. Crystal materialization remains a separate later stage. RAG v1.1 (the embedding/semantic ranker) should still follow the absorb boundary — now that notes carry real content, the ranker has something to rank.
