...
**M27–M31 — Crystal v2, product console, and the daily product line (Level-2 push).** M27 expanded Crystal to v2 (12 durable / 14 caveated); M28 built the product-facing bilingual Crystal Console (Python over `.run`, superseded as a product surface by M31). **M30/M31 made the reader/crystal trunk the blessed DAILY workflow on the real vault**, adding four product crates that depend only on the trunk (never the demoted substrate — CI-gated):

- **`ovp-intake`** — the capture boundary: sweeps `Clippings/` + `50-Inbox/00-Capture` + `50-Inbox/02-Pinboard` into normalized `50-Inbox/01-Raw/<YYYY-MM>/` files with URL + content-sha256 dedup (duplicates parked, never deleted; thin/broken files flagged in place, hash-keyed). Pinboard capture is a `PinboardFetch` trait: JSON-export fixture impl always compiled; live HTTP behind the `pinboard-live` feature (`PINBOARD_TOKEN` env, errors token-safe). Also owns the shared vault-state primitives (`vaultops`: append-only JSONL ledgers, `safe_move`/`write_new` that never overwrite, the OVP_RULES `pipeline.jsonl` write-log event, `RunLock`).
- **`ovp-daily`** — the daily loop: plan (hash dedup vs the append-only `.ovp/daily-runs.jsonl` ledger; 3 failures ⇒ blocked) → reader trunk per source → packs in `40-Resources/Reader/` → lifecycle move to `03-Processed/` (strictly AFTER the success record is durable) → per-run report in `.ovp/reports/`. Audit ordering is fault-injection-tested: write → write-log event → ledger record.
- **`ovp-index`** — the read model: a deterministic JSON projection (`.ovp/index/index.json`) folded from the ledgers, a raw-inbox scan, the reader packs, the crystal store, and the run reports; full rebuild every time (rebuild IS the migration story; **decision: no SQLite/FTS/embeddings** until daily query pain proves the need). Queried by `find`.
- **`ovp-console`** — the product console: one deterministic bilingual (EN + 中文) HTML page over the read model (`.ovp/console/index.html`): attention feed, runs, sources, reader packs, crystal durable/caveated — every row linking back to its provenance artifact. Reads PRODUCT state only, never `.run`.

`ovp-cli daily` composes the phases (pinboard → intake → plan/read → lifecycle → report → index → console); every CLI verb is labeled **PRODUCT** / **DIAGNOSTIC** / **DEMOTED** in `--help`. This closed the M29 P0 set (intake, lifecycle movement, blessed write path, durable ledger/audit, read-index decision) — see `docs/stage-m31-mainline-capability-closure.md`, `docs/operator-runbook.md`, and `docs/product-state-layout.md`.
