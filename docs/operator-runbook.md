...
After 3 stable manual days, put the daily on the OS scheduler — this is a
product command, not a hand-rolled cron entry:

```bash
ovp2 schedule install --vault-root "$VAULT"          # daily at 09:00
ovp2 schedule install --vault-root "$VAULT" --time 09:30 --max-sources 5
ovp2 schedule status                                 # loaded? last run? stale?
ovp2 schedule uninstall                              # removes the job; keeps logs + env
```

What `install` does:

- **macOS**: writes `~/Library/LaunchAgents/com.ovp2.daily.plist` (chmod 600)
  and `launchctl load`s it. **Linux**: writes
  `~/.config/systemd/user/ovp2-daily.service` + `.timer`
  (`OnCalendar`, `Persistent=true`) and enables the timer. **Windows**:
  registers a Task Scheduler task `OVP2 Scheduler` (`schtasks /Create`, every
...
- On macOS/Linux the job runs via `/bin/sh -c` and sources an **env file** first
  ...
  `install` creates a chmod-600 template with commented
  `ANTHROPIC_API_KEY` / `ANTHROPIC_BASE_URL` / `OVP_LLM_MODEL` /
  `GITHUB_TOKEN` lines (and `OVP_LLM_TIMEOUT_SECS=480` set) and prints what
  to fill in. Credentials live only there — never in the unit file.
- The invocation is `ovp2 daily --vault-root <vault> --client live
  --date "$(date +%F)"` (the local-date trap from §1 is handled) plus
  `--web-fetch-live --github-live` enrichment by default (`--no-enrich` to
  turn off) and `--max-sources` / `--client replay` passthrough.
- Output goes to `<vault>/.ovp/logs/daily-launchd.log` (macOS) /
  `daily-systemd.log` (Linux); `status` tails the last lines and warns when
  the last ledger run is more than 2 days old.
- The job pins the `ovp2` binary path resolved at install time — re-run
  `ovp2 schedule install` after moving the binary (brew upgrades keep the
  path stable). Re-install is idempotent: it unloads and overwrites cleanly.
- The interim hand-written plist on the operator machine uses the same label
  `com.ovp2.daily`, so `ovp2 schedule install` supersedes it cleanly
  (unload + overwrite). Generated files carry a "generated by ovp2 schedule —
  do not hand-edit" header; hand edits are lost on the next install.
- TODO: the portal System page does not yet show scheduler state —
  `/api/settings` is vault-only display data and reading launchd/systemd
  would need platform code in ovp-server.

Notes: exit code is non-zero when ANY source fails (partial success included) —
the scheduler log, not the exit code, is the thing to read; check
`.ovp/daily-runs.jsonl` (or just `ovp2 schedule status`) and run `ovp2 doctor`
weekly. A stale `run.lock` from a crashed run is reclaimed automatically
(dead-PID probe); a lock held by a live run makes the scheduled run exit with
"another OVP run appears to be in progress" — that is correct behavior, not a
failure to fix.
...
- No daemon — scheduling is the OS's job: `ovp2 schedule install` (§1) puts
  `daily` on launchd/systemd.
