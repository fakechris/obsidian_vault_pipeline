...
| M5 Context Pack And Operational Runtime | Active | session snapshot、context budget、第一版 operational runtime state projection、claim lease、provider facade、observability |
...
- 当前：`BL-014` 从 `ovp-runtime-state` 开始，把 repair marker、pipeline event、trusted reuse event 投影成可见 runtime state。
