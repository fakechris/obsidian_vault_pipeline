...
- 当前：M5 已收口。`BL-014` 让 workflow action 进入 runtime state 可观测范围，但不引入通用 workflow lease；现有 action worker lock 和 stale-running 状态继续作为执行边界，直到真的需要多 worker 调度。
...
| `ovp-runtime-state --vault-dir <vault> --write --json` | 从 repair marker、workflow action、pipeline event、reuse event 生成 operational runtime state projection，写入 `60-Logs/runtime-state/current.{json,md}` |
| `GET /api/runtime-state` | 本地读取 provider-facing runtime-state projection；优先读已物化的 `60-Logs/runtime-state/current.json`，缺失时才重建 |
| `POST /api/runtime-state` | 刷新并写入已物化的 runtime-state projection |
