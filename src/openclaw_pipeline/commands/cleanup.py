...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate cleanup proposals for evergreen notes")
    ...
    parser.add_argument("--write", action="store_true", help="Apply deterministic cleanup mutations")
    ...
    if args.write:
        mutations = [
            execute_cleanup(target, proposal, write=True)
    ...
    if args.json:
        ...
        print(f"cleanup proposals: {len(proposals)}")
        for proposal in proposals:
            print(f"- {proposal['slug']}: {proposal['action']}")
        if args.write:
            print(f"cleanup applied: {payload['applied_count']}")
    return 0
