...
| M16 Reader / Maintainer Surface Split | Active | Hard-split the UI shell into a Reader product (`/`, `/search`, `/atlas`, `/atlas/curated`, `/object`, `/note`, `/topic`, `/map`, `/explore`) and a Maintainer product (`/ops/...`).  URL prefix decides shell; old top-level maintainer paths 301 to `/ops/<same>`.  Reader home surfaces M14 substrate (Top Topics + Curated Atlas + Recent Crystals) and drops DB stats / Workbench card.  See `docs/plans/2026-05-04-bl-050-reader-maintainer-split.md` |
...
| BL-050 | P0 | Active | Reader / Maintainer hard split — URL prefix decides shell, Reader home rewritten around `crystal_scores` + Curated Atlas + Recent Crystals, 27 maintainer routes relocated under `/ops/*` with 301 back-compat, `?mode=operator` toggle removed, single cross-link in each shell footer.  See `docs/plans/2026-05-04-bl-050-reader-maintainer-split.md` | M16 |
