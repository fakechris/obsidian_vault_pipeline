...
| BL-051 | P0 | Active | Reader vocabulary unification + map cleanup — `/atlas/curated` → `/topics` (301 back-compat), page title `Curated Atlas` → `Featured Topics`, FTS prefix `[crystal]` → `[topic]` and `[contradiction]` → `[open question]`, Reader nav `Atlas` link → `Topics`, home drops the redundant Curated Atlas card and adds `See all N featured topics →` link inside Top Topics, `Recent Crystals` → `Recent Topics`, `/map` capped at 3 members per cluster with `?show_all=1` escape hatch + hover-only labels.  Internal storage names (tables, files, CLI verbs, frontmatter `type:`) untouched.  See `docs/plans/2026-05-04-bl-051-vocabulary-and-map-cleanup.md` | M16 |
| BL-052 | P1 | Audit | Maintainer Vocabulary Audit — produce a matrix of every `/ops/*` surface mapping URL → renderer → view-model → backing entity → overlap-classification (R / C / N / K).  Audit-only deliverable; no code changes.  Implementation plan (BL-053) derived from the audit.  See `docs/plans/2026-05-04-bl-052-maintainer-vocab-audit.md` | M16 |
