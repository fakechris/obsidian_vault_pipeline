...
| M14 Curated Atlas (Crystal Read Model) | Done | `crystal_scores` Projection (#142) + Curated Atlas markdown export (#143) + page_fts indexing of crystal bodies (#144) + big-community splitter inside `_detect_communities` (#145) + reuse-feedback wired into BL-045 scoring (BL-045/046/047/048/049). Tag + entity facets and the `/atlas/curated` HTTP route deferred to small follow-ups. See `docs/plans/2026-05-04-m14-curated-atlas.md` |
...
| BL-046 | P0 | Done | Curated Atlas Access Surface — markdown export half: `ovp-build-curated-atlas` writes `40-Resources/CuratedAtlas.md` with the top-N crystals (default 30) ranked by `crystal_scores`.  Each entry carries label, teaser, full score breakdown, and a wikilink to the underlying crystal markdown.  Atomic write via `os.replace`; takes the `knowledge_db_write_lock`.  HTTP `/atlas/curated` route deferred to a small follow-up. | M14 |
