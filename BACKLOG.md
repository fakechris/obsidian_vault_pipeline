...
| BL-053 | P1 | Next | Maintainer Vocabulary Cleanup (implementation) — apply the renames recommended by the BL-052 matrix.  In priority order: nav `Audit` → `Events`; `Evolution Candidates` → `Relation Proposals` (URL move); `Stale Summaries` → `Compiled Summaries` page title; `Open Questions` fragment → `Query Followups`; `Workbench` → `Review Queue`; `Production` → `Source Production Chain`; doc updates for Compiled-Content trio. | M16 |
