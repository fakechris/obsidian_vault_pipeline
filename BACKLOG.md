...
| BL-053 | P1 | In Progress (Phase 1 done) | Ops 工作台 IA 重构 + 词汇清理.  **Phase 1 (this PR)**: by-time pivots ahead of the toolbox — new `/ops/today` (5-card daily digest: intake/absorb/synthesis/governance/failures), new `/ops/runs` index + `/ops/runs/<txn_id>` per-run timeline (joins via session_id since most events don't carry txn_id directly).  Nav rebuilt: `Today` and `Runs` ahead of `Timeline`; `Audit` → `Events`; `Candidates` → `Concept Candidates`.  **Phase 2 (deferred)**: `/ops/timeline?view=workflow` matrix, `/ops/health`, `/ops/forensics?source=`, queue grouping (`/ops/queue/*`), URL renames (`/ops/evolution` → `/ops/queue/relation-proposals`), remaining BL-052 vocab fixes (Stale Summaries, Workbench, Production). | M16 |
...
| BL-056 | P0 | Active | Stage emit hooks — every stage that creates / modifies a Canonical-State object writes a provenance row beyond the rebuild's ingest baseline.  Wired: `synthesize_community_crystal`, `synthesize_contradiction_crystal`, `promote` (concept review).  Shared helper at `src/ovp_pipeline/provenance.py`.  See `docs/plans/2026-05-04-bl-056-stage-emit-hooks.md` | M14 |
