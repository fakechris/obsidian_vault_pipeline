...
- runtime state 现在是一层派生的 operational projection：`ovp-runtime-state` 读取 projection repair marker、pipeline event、trusted reuse event，并写出带 graph nodes/edges 与 attention items 的 `60-Logs/runtime-state/current.{json,md}`。
...
4. 继续 BL-014：把 runtime state 接入 `/ops`、`ovp doctor` 和 provider-facing read API，但不能把 projection 当成 Authority。
...
| Operational runtime graph / observability | `BL-014` 第一片 runtime-state 在 PR #91 落地 |
