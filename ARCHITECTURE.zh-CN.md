...
- runtime state 现在是一层派生的 operational projection 和读取 surface：`ovp-runtime-state` 写出 `60-Logs/runtime-state/current.{json,md}`，`/ops` 用同一份 projection 展示 system health，`ovp doctor` 输出同一组 metrics，`/api/runtime-state` 暴露 provider-facing read API。它也会追踪 workflow action queue health；workflow action 继续使用现有 action worker lock 加 stale-running attention，不在多 worker 调度出现前抽象成通用 lease。
