...
- runtime state 现在是一层派生的 operational projection 和读取 surface：`ovp-runtime-state` 写出 `60-Logs/runtime-state/current.{json,md}`，`/ops` 用同一份 projection 展示 system health，`ovp doctor` 输出同一组 metrics，`/api/runtime-state` 暴露 provider-facing read API。
...
4. 收口 BL-014：判断 workflow item lease 是否真的需要从 projection repair marker 扩展出去；runtime state 必须保持为 projection，不能变成 Authority。
...
| Operational runtime graph / observability | `BL-014` 第一片 runtime-state 在 PR #91 落地；`/ops` / doctor / API wiring 在 PR #92 |
