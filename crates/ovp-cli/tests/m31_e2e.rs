...
fn daily_enrich_logs_source_enriched_and_prints_daily_result_line() {
    ...
    // 0, not the note's length: the reader's note now lives in the
    // `annotation:` frontmatter key and the bookmark body is written empty
    // (#481), so there is no prior body for enrichment to overwrite.
    assert_eq!(field("old_body_chars"), "0");
...
fn full_daily_workflow_capture_to_console_with_crystal_and_retry() {
    ...
    // The machine-readable line must agree with the human one: annotations
    // moving out of the body changed this run's count from 2 to 1.
