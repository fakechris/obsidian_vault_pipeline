...
#[cfg(any(feature = "web-fetch-live", test))]
fn normalize_xquik_api_key(key: String) -> Option<String> {
    let key = key.trim();
    ...
    pub fn new(config: WebFetchConfig) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            xquik_api_key: std::env::var("XQUIK_API_KEY")
                .ok()
                .and_then(normalize_xquik_api_key),
        })
    }
...
mod tests {
    ...
    #[test]
    fn xquik_api_key_is_trimmed_and_empty_values_are_disabled() {
        assert_eq!(
            normalize_xquik_api_key("  secret  ".to_string()).as_deref(),
            Some("secret")
        );
        assert_eq!(normalize_xquik_api_key(" \t\n ".to_string()), None);
    }
