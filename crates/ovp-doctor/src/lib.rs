//! Vault health checks — the engine behind `ovp2 doctor`.
//!
//! Checks: ledger↔fs consistency, orphan packs, stale index, crystal ledger
//! integrity, crystal staleness, run recency, per-dir disk usage, legacy
//! artifacts, inbox orphans. [`run_checks`] is read-only unless
//! `DoctorOptions::fix` is set, and then applies safe repairs only (rebuild
//! index, quarantine orphans — never deletes).
//!
//! This is a library so the CLI, the portal server and the MCP `doctor` tool
...
use ovp_domain::diagnostics::{self, Diagnostic, Origin};
...
/// What to run. `fix` applies safe repairs; `since_hours` overrides the
/// run-recency threshold ([`DEFAULT_RECENCY_HOURS`]).
...
impl Finding {
    ...
    /// Project onto the shared [`Diagnostic`] shape. `Pass` is not a
    /// diagnostic (nothing to act on) and yields `None`; a finding that
    /// `--fix` already repaired is reported as `Info` so the reader sees what
    /// happened without being told to act.
    pub fn to_diagnostic(&self) -> Option<Diagnostic> {
        let severity = match (&self.severity, self.fixed) {
            (Severity::Pass, _) => return None,
            (_, true) | (Severity::Info, _) => diagnostics::Severity::Info,
            (Severity::Warn, _) => diagnostics::Severity::Warning,
            (Severity::Fail, _) => diagnostics::Severity::Error,
        };
        let message = if self.fixed {
            format!("{} [FIXED]", self.message)
        } else {
            self.message.clone()
        };
        Some(Diagnostic {
            origin: Origin::Doctor,
            severity,
            code: format!("doctor.{}", self.check),
            message,
            location: None,
            hint: if self.fixed { None } else { self.hint.clone() },
        })
    }
}

/// Convert a whole run into a [`diagnostics::DiagnosticReport`] (passes dropped).
pub fn to_diagnostics(findings: &[Finding]) -> diagnostics::DiagnosticReport {
    diagnostics::DiagnosticReport::new(findings.iter().filter_map(Finding::to_diagnostic).collect())
}

/// Run every check in the fixed order `ovp2 doctor` prints them.
pub fn run_checks(vault_root: &Path, opts: &DoctorOptions) -> Vec<Finding> {
    let layout = VaultLayout::new();
    let mut findings: Vec<Finding> = Vec::new();

    check_ledger_fs_consistency(vault_root, &layout, &mut findings);
    check_orphan_packs(vault_root, &layout, &mut findings);
    check_stale_index(vault_root, &mut findings, opts.fix);
    check_crystal_integrity(vault_root, &mut findings);
    check_crystal_staleness(vault_root, &mut findings);
    let threshold_hours = opts.since_hours.unwrap_or(DEFAULT_RECENCY_HOURS);
    check_run_recency(
        vault_root,
        &layout,
        now_unix_secs(),
...
pub fn today_civil() -> (i32, u32, u32) {
    use chrono::Datelike;
    let now = chrono::Local::now().date_naive();
    (now.year(), now.month(), now.day())
}

fn check_ledger_fs_consistency(
    vault_root: &Path,
    layout: &VaultLayout,
        ...
        (Some(idx), Some(led)) if idx < led => {
            ...
            "the rebuild itself failed — read the error above; `find`, the portal and the console all read this projection, so they stay stale until it succeeds",
                ...
                findings.push(Finding {
                    check: "stale-index".into(),
                    severity: Severity::Warn,
                    message: "index.json is older than the daily ledger (run `index` or `daily`)"
                        .into(),
                    hint: None,
                    fixed: false,
                });
...
fn check_crystal_staleness(vault_root: &Path, findings: &mut Vec<Finding>) {
    ...
    let report =
        match ovp_domain::crystal::recheck::recheck_vault(vault_root, None, None, today_civil()) {
            Ok(r) => r,
            Err(e) => {
                let finding = Finding {
...
const LEGACY_ARTIFACTS: &[(&str, &str)] = &[
    (
        "knowledge.db",
        "legacy SQLite projection (non-standard location)",
    ),
    ("60-Logs/knowledge.db", "legacy SQLite projection"),
    (
        "60-Logs/knowledge.db.lock",
        "legacy knowledge.db write lock",
    ),
    (
        "60-Logs/backups",
        "legacy knowledge.db snapshot directory (ovp-backup-db)",
    ),
    (
        "60-Logs/signals.jsonl",
        "legacy source-authority signals log",
    ),
    ...
    (
        "60-Logs/transactions",
        "legacy TransactionManager journal directory",
    ),
    ("60-Logs/derived", "legacy derived-artifacts directory"),
    (
        ".ovp/llm_profiles.yaml",
        "legacy ovp-ask LLM provider profiles",
    ),
    (".ovp/digest.yaml", "legacy digest config"),
    (
        ".ovp/schema_version",
        "legacy knowledge.db schema-version marker",
    ),
];
...
mod tests {
    ...
    fn actionable_findings_carry_a_hint_or_name_the_command_themselves() {
        ...
        assert!(
            actionable >= 10,
            "only {actionable} actionable finding(s) reached"
        );
        assert!(
            exempted >= 1,
    ...
    fn a_finding_does_not_repeat_its_command_in_both_message_and_hint() {
        for (label, vault) in actionable_fixtures() {
            for f in all_checks(vault.path()) {
                let Some(hint) = f.hint.as_deref() else {
                    continue;
                };
                assert!(
                    !names_a_command(&f),
    ...
    fn inbox_orphans_flags_unmanaged_dirs_only() {
        ...
        assert!(
            f.message.contains("3 md file(s)"),
            "READMEs excluded: {}",
            f.message
        );
    ...
    fn legacy_artifacts_reports_python_era_files_as_info() {
        ...
        for f in &findings {
            ...
            assert_eq!(f.severity, Severity::Info, "{}", f.message);
            assert!(
                f.message.contains("Python-era OVP artifact"),
                "{}",
                f.message
            );
            assert!(
                f.message
                    .contains("github.com/fakechris/obsidian_vault_pipeline"),
                "{}",
                f.message
            );
        ...
        assert!(
            messages
                .iter()
                .any(|m| m.starts_with("60-Logs/knowledge.db "))
        );
        assert!(
            messages
                .iter()
                .any(|m| m.starts_with("60-Logs/transactions "))
        );
        assert!(
            messages
                .iter()
                .any(|m| m.starts_with(".ovp/llm_profiles.yaml "))
        );
    }
    ...
    fn info_findings_do_not_fail_doctor() {
        ...
        let findings = run_checks(tmp.path(), &DoctorOptions::default());
        assert!(
            findings
                .iter()
                .any(|f| f.check == "legacy-artifacts" && f.severity == Severity::Info)
        );
        assert!(
            !findings
                .iter()
                .any(|f| f.severity == Severity::Fail && !f.fixed),
            "INFO finding must not fail doctor: {findings:?}"
        );
        // Same rule on the shared projection: Info never becomes Error, and
        // passes are not diagnostics at all.
        let report = to_diagnostics(&findings);
        assert!(
            report.passed(ovp_domain::diagnostics::Severity::Error),
            "{report:?}"
        );
        assert!(
            report
                .diagnostics
                .iter()
                .all(|d| d.code.starts_with("doctor."))
        );
        assert_eq!(
    ...
    fn recency_findings(vault: &Path, now: i64, threshold: u64) -> Vec<Finding> {
        let layout = VaultLayout::new();
        let mut f = Vec::new();
    ...
    fn recency(f: &[Finding]) -> &Finding {
        f.iter()
            .find(|x| x.check == "run-recency")
            .expect("a run-recency finding")
    }
    ...
    fn recency_parses_rfc3339() {
        ...
        assert_eq!(
            parse_rfc3339_utc("2026-07-12T09:00:00Z"),
            Some(1_783_846_800)
        );
    ...
    fn recency_fails_on_stale_completed_heartbeat() {
        ...
        assert_eq!(
            recency_findings(tmp.path(), future, 26)[0].severity,
            Severity::Fail
        );
    }
