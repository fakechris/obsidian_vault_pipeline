...
| `ovp-cli` | Thin arg-parsing layer: builds `ModelClient` + `ConceptRegistry` + `AppWiring`, delegates assembly to `ovp-app`, the cycle to `ovp-run`, reads to `ovp-query` / `ovp-lint` / `ovp-rag`, the sweep to `ovp-auto`, the review pack to `ovp-review`. Subcommands: `run-cycle`, `query`, `lint`, `rag`, `auto-run`, `review-run`, `interpret-article` (`--client replay|live`), `apply-plan`, `graph`. |
