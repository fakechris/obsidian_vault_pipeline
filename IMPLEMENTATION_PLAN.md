...
```
OVP2 Portal
├── 工作台 Dashboard    今日 runs、attention feed、统计卡（M28 Attention 风格）
├── 管线 Pipeline（每步一个入口，页面直接对应 CLI 步骤）
│   ├── 捕获 Capture    pinboard 同步状态、clippings 队列、needs-content
│   ├── 摄入 Intake     intake ledger、去重/parked、生命周期
│   ├── 阅读 Reader     packs 列表 + 详情（cards/units/溯源）
│   ├── 结晶 Crystal    claims（durable/caveated）+ M33 viz 图谱挂此处
│   ├── 复核 Review     review 队列、session 入口
│   └── 产出 Project    vault notes 投影、digest
├── 运行 Ops            run 报告、doctor、blocked 源
├── 概念 Concepts       关键概念说明（durable/caveated/unit/gate…，M28 About 升级版）
└── 设置 Settings       vault 路径、LLM client、caps、语言（先只读展示，再逐步可改）
```
- [ ] 实现方式定案（见开放问题 Q5：M28 风格服务端渲染 vs 扩展 M33 SPA vs 混合）。
...
5. **Q5 portal 实现方式**：(a) M28 风格服务端渲染多页（最快，设计现成，~2-3 天）；
   (b) 全部并入 M33 React SPA（最现代，~5-7 天，挤压 merge 窗口）；
   (c) 混合：服务端渲染做壳 + 导航 + 各步入口，viz SPA 嵌在 Crystal 下（推荐，~3-4 天）。
