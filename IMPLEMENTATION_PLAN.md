...
```
OVP2 Portal（主导航 = 四个用户任务）
├── 今天 Today       今天进来了什么、结晶了什么（digest 上屏 + attention 仅在需要人时出现）
│                    系统健康 = 右上角一颗状态灯，不是一个页面
├── 搜索 Search      全库查询：sources/cards/claims/units，结果带证据与溯源跳转
├── 知识 Knowledge   结晶层浏览：durable claims 按主题组织，M33 viz 图谱挂此处；
│                    每条 claim 可下钻到 card→unit→原文行号
