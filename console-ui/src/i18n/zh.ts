...
export const zh: Record<keyof typeof en, string> = {
  ...
  'banner.runningProgress': '运行进行中 · {done}/{total}（{current}）· 开始 {when}（{ago}）',
  'banner.runningProgressNoCurrent': '运行进行中 · {done}/{total} · 开始 {when}（{ago}）',
  'banner.stale': '最近运行：{when}（{ago}）—— 每日流程可能已停滞',
  'banner.failed': '最近运行：失败 · {when}（{ago}）{error}',
  'banner.aborted': '最近运行：中断 · {when}（{ago}）{error}',
  ...
  'activity.title': '运行活动',
  'activity.running': '进行中 · {done}/{total} · {pct}% · 开始 {when}（{ago}）',
  'activity.current': '当前：{current}',
  'activity.idle': '当前没有运行。显示上一次运行的时间线。',
  'activity.empty': '没有逐条来源记录（未进入 reader，或计划处理数为 0）。',
  'activity.emptyAfterFail':
    '没有逐条来源记录 —— 运行在处理任何来源之前就失败了（见上方时间线）。intake/reader 未启动。',
  'activity.emptyRunning': '等待第一条来源结果…（阶段完成时时间线会更新）',
  'activity.feedTitle': '逐条来源（最新在上）',
  ...
  'activity.finishedOk': '完成 {started} → {ended} · {ok} 已读 · {failed} 失败',
  'activity.finishedFail': '失败 {started} → {ended} · 退出前 {ok} 已读 · {failed} 失败',
  'timeline.title': '时间线',
  'timeline.noClock': '—',
  'timeline.stillRunning': '（仍在运行）',
  'timeline.clockLine':
    '{runId} · {status} · 开始 {started} · 结束 {ended} ·（{ago}）',
  'timeline.schedStamp': '写入调度戳（{status}）',
  'timeline.started': 'daily 进程启动（{runId}）',
  'timeline.completed': '完成 · {ok} 已读 · {failed} 失败 · 队列剩余 {queued}',
  'timeline.running': '仍在运行（抓取/规划阶段 — 尚无来源计数）',
  'timeline.runningProgress': '正在阅读来源 · {done}/{total}{current}',
  'timeline.failPinboard':
    '失败于 Pinboard 抓取 — intake 与 reader 从未启动。{error}',
  'timeline.failIntake': '失败于 intake 扫荡 — reader 从未启动。{error}',
  'timeline.failEnrich': '失败于补全（web/github）。{error}',
  'timeline.failReader': '失败于 reader / LLM。{error}',
  'timeline.failIndex': '失败于索引/控制台刷新。{error}',
  ...
  'theme.filterLabel': '状态筛选',
  'theme.filterAll': '全部',
  'theme.filterDurable': '持久',
  'theme.filterCaveated': '存疑',
  'theme.sortLabel': '主张排序',
  'theme.sortDefault': '默认',
  'theme.sortDay': '按日期',
  'theme.filterEmpty': '该主题下没有【{filter}】主张。',
  'theme.claimDayTip':
    '主张写入 crystal 账本的日期；若其运行无日期（早期 sweep），则取其所引用的最新源在流水线中的最近处理日期。',
  ...
  'ask.historyVault': '全库',
  'ask.collapseHistory': '收起历史',
  'ask.expandHistory': '展开历史',
  'ask.collapseRail': '收起过程与引用',
  'ask.expandRail': '展开过程与引用',
  'ask.railCollapsedLabel': '过程',
  'ask.savedChat': '已保存会话',
  'ask.continuing': '继续对话中',
  'ask.closeChat': '返回对话',
  ...
  'knowledge.sortUpdated': '更新',
  'knowledge.sortCreated': '创建',
  'knowledge.themeCreated': '创建 {day}',
  'knowledge.themeUpdated': '更新 {day}',
  'knowledge.themeTimesTip':
    'B 轴日期取自各主张的账本运行日（运行无日期时用其最新引用源的最近流水线处理日）：创建 = 主题最早的主张日期，更新 = 最新的主张日期。',
