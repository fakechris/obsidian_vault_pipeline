...
def test_installer_loads_ovp_scripts_from_pyproject():
    from ovp_pipeline.installer import load_project_scripts

    scripts = load_project_scripts(Path(__file__).resolve().parents[1])

    assert scripts["ovp"] == "ovp_pipeline.unified_pipeline_enhanced:main"
    assert scripts["ovp-ui"] == "ovp_pipeline.commands.ui_server:main"
    assert scripts["ovp-truth"] == "ovp_pipeline.commands.truth_api:main"


def test_installer_writes_executable_shims(tmp_path):
    from ovp_pipeline.installer import write_shims
    ...
    assert "from ovp_pipeline.commands.ui_server import main" in body
...
def test_installer_shell_escapes_python_executable(tmp_path):
    ...
    target_dir = tmp_path / "bin"
    scripts = {"ovp-ui": "ovp_pipeline.commands.ui_server:main"}
...
def test_choose_install_bin_dir_prefers_user_path_entry(tmp_path):
    from ovp_pipeline.installer import choose_install_bin_dir
