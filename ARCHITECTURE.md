...
- Runtime state is now a derived operational projection and read surface: `ovp-runtime-state` writes `60-Logs/runtime-state/current.{json,md}`, `/ops` renders system health from the same projection, `ovp doctor` includes the same metrics, and `/api/runtime-state` exposes the provider-facing read API. It also tracks workflow action queue health; workflow actions use the existing action worker lock plus stale-running attention instead of a generalized lease until multi-worker scheduling exists.
